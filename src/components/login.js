      ...
      <div class="relative bg-[#212121] p-8 rounded-lg shadow-lg w-full max-w-md border border-white/10">
        ...
        <form id="login-form" class="flex flex-col gap-4">
          <div>
            ...
            <input type="email" id="email" required class="w-full bg-[#121212] border border-gray-600 rounded p-2.5 text-white focus:border-white outline-none focus:ring-1 focus:ring-white transition" placeholder="name@example.com">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-400 mb-1">Mật khẩu</label>
            <div class="relative">
                <input type="password" id="password" required class="w-full bg-[#121212] border border-gray-600 rounded p-2.5 pr-10 text-white focus:border-white outline-none focus:ring-1 focus:ring-white transition" placeholder="••••••••">
          ...
          <button type="submit" class="mt-2 w-full bg-white text-black font-bold py-2.5 rounded-full hover:bg-gray-200 transition transform active:scale-95">Đăng nhập</button>
        </form>
        <p class="mt-6 text-center text-gray-400 text-sm">Chưa có tài khoản? <a href="#/register" class="text-white font-medium hover:underline">Đăng ký ngay</a></p>
      </div>
    </div>`;
}
    ...
    form.querySelectorAll('.toggle-password').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (input.type === 'password') {
                ...
                btn.innerHTML = iconEyeHide; // Đổi sang icon gạch chéo
            } else {
                input.type = 'password'; // Ẩn mật khẩu
                btn.innerHTML = iconEyeShow; // Đổi về icon mắt thường
            }
        });
    });
