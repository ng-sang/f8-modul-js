
export function explore1() {
   return ` <a href="/explore" class="mb-2 flex w-full flex-col items-center justify-center rounded-lg py-4 text-[#909090] transition-colors hover:bg-[#2b2b2b] hover:text-white">
          <svg class="mb-1 h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-5.5-2.5l7.51-3.49L17.5 6.5 9.99 9.99 6.5 17.5zm5.5-6.6c.61 0 1.1.49 1.1 1.1s-.49 1.1-1.1 1.1-1.1-.49-1.1-1.1.49-1.1 1.1-1.1z"></path>
          </svg>
          <span class="text-[10px] font-medium tracking-wide">Khám phá</span>
        </a>`
}